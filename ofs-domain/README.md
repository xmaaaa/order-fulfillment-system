...
| [ROADMAP.md](ROADMAP.md) | 已具备能力清单、后续待补齐、与 ofs-app 装配要点 |
...
在 IDE 中运行 `com.ofs.domain.ScenarioDemo#main`，或：

```bash
mvn compile exec:java -pl ofs-domain -Dexec.mainClass="com.ofs.domain.ScenarioDemo"
```
...
- **支付、库存**：以防腐层形式存在（`PaymentClient`/`InventoryClient` 接口），由 ofs-app 提供实现。
