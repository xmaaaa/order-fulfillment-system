# xm-ai：订单智能助手（Spring AI + DeepSeek）
...
**模块为何独立**：主系统绑着 spring-cloud-alibaba/Sentinel 的固定版本，直接加 Spring AI 会版本打架。`xm-ai` 用自己的 `<parent>`（boot 3.4.5），只通过根 pom 的 `<modules>` 聚合、不继承根 pom → 既被 IDE/Maven 收纳，又版本隔离。
...
```bash
# 启动 AI 应用（:8080）。工具调用/agent 相关功能还需订单系统 xm-spring(:8888) 一起跑。
cd xm-ai && mvn spring-boot:run
```
...
| 现象 | 原因 | 解法 |
|---|---|---|
| 启动报 `DeepSeek API key must be set` | key 文件靠"工作目录"读取，IDE 工作目录是项目根不是 xm-ai | 把 `application-local.yml` 放 **classpath**(`src/main/resources`) |
